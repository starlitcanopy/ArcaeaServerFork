...
def header_check(request) -> ArcError:
    ...
    if request.method == 'GET' and 'ContentBundle' in headers and headers['ContentBundle'] != BundleParser.max_bundle_version.get(headers.get('AppVersion', ''), '0.0.0'):
        current_app.logger.warning(f"Max version: {BundleParser.max_bundle_version.get(headers.get('AppVersion', ''), '0.0.0')}")
        current_app.logger.warning(f"Content bundle header: {headers['ContentBundle']}")
