...
class MapParser:
    ...
    def parse(self) -> None:
        ...
        for i in range(4):
            self.map_lephon_nell_phases[i] = os.path.join("./database/map_lephon_nell", f"{i+1}.json")
...
class Map:
    ...
    def from_dict(self, raw_dict: dict) -> 'Map':
        ...
        self.steps = self.overwrite_steps if self.overwrite_steps != None else [Step().from_dict(s) for s in raw_dict.get('steps')]
...
class UserMap(Map):
    ...
    def rewards_for_climbing(self) -> list:
        rewards = []
        for i in range(min(self.prev_position, self.curr_position)+1, max(self.prev_position, self.curr_position)+1):
            step = self.steps[i]
            if step.items:
                rewards.append(
    ...
    def steps_for_climbing(self) -> list:
        return self.steps[min(self.prev_position, self.curr_position):max(self.prev_position, self.curr_position)+1]
    ...
    def select(self):
        ...
        if x.step_type:
            if not self.lephon_final and ("wall_nell" in x.step_type or "wall_impossible" in x.step_type):
